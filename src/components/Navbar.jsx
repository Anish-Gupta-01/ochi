...
export default function Navbar() {
  ...
  return (
    <>
    <div className={`fixed top-0 z-50 w-full px-6 md:px-10 py-4 md:py-8 
      font-[Neue_montreal, Roboto,  sans-serif] flex justify-between items-center shadow-md transition-transform duration-300 ${isNavbarVisible ? "transform translate-y-0" : "transform -translate-y-full"}`}>
      <div className='logo'>
      <svg width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="link hidden md:flex gap-6 lg:gap-10">
            {["Services", "Our Work", "About Us", "Insights", "Contacts us"].map((item, index) => 
            <a key={index} className={`text-sm md:text-lg capitalize font-light ${index === 4 && "ml-10 lg:ml-56"}`}>{item}</a>)}
        </div>
      ...
      <div className="md:hidden flex flex-col fixed z-[998] w-full py-5 px-8 shadow-md">
        {["Services", "Our Work", "About Us", "Insights", "Contacts"].map((item, index) => (
          <a key={index} className="text-base font-light py-2 border-b border-gray-300">
            {item}
          </a>
        ))}
    </div>
